...
export class CalendarComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataService.calendarDate()
      .subscribe(
            data => {
              this.viewDate = startOfDay(new Date());
              for(var i=0;i<data.length;i++) {
                var sdt = new Date(data[i].interviewTime);
                var edt = new Date(data[i].interviewTime);
                var tH = sdt.getHours(); var tM = sdt.getMinutes();
                var ttl = 'Interview with ' + data[i].jobOffer.company + '. Mode: ' + data[i].interviewMode + '. Details: ' + data[i].interviewModeDetail + '. Time: ' + tH + ' : ' + tM;
                  this.events.push( {
                    start: sdt,
                    title: ttl,
                    color: colors.def,
